#!/usr/bin/env python3
"""
Back-project LiDAR points to camera image using KITTI calibration data
This script generates a camera view image with LiDAR points projected onto it
"""
...
def project_lidar_to_camera(lidar_points, calib_data, camera_id=2):
    ...
    Args:
        ...
        camera_id: Camera to project to (0, 1, 2, or 3)
    
    Returns:
        image_points: Nx2 array of image coordinates
        depths: N array of depths for coloring
        valid_mask: Boolean mask for points within image bounds
    """
...
def create_lidar_image(image_points, depths, image_size=(1242, 375), max_distance=80):
    ...
    Returns:
        image: Generated image with LiDAR points
    """
    ...
    # Use a colormap (e.g., jet colormap)
    colors = cm.jet(1 - normalized_depths)  # Invert so close = red, far = blue
...
def add_depth_colorbar(image, max_distance=80):
    """Add a colorbar showing depth scale"""
...
def main():
    """Main function to generate LiDAR back-projection image"""
    ...
    output_file = '/home/jal.parikh/yolo3d/YOLO3D/lidar_projection_000002.png'
    ...
    # Print some statistics about the LiDAR data
    print(f"\nLiDAR Point Cloud Statistics:")
    print(f"X range: {lidar_points[:, 0].min():.2f} to {lidar_points[:, 0].max():.2f}")
    print(f"Y range: {lidar_points[:, 1].min():.2f} to {lidar_points[:, 1].max():.2f}")
    print(f"Z range: {lidar_points[:, 2].min():.2f} to {lidar_points[:, 2].max():.2f}")
    
    # Project to camera 2 (left color camera)
    print("\nProjecting LiDAR points to camera image...")
    image_points, depths, front_mask = project_lidar_to_camera(lidar_points, calib_data, camera_id=2)
    
    print(f"Projected {len(image_points)} points to image")
    
    if len(image_points) > 0:
        print(f"Depth range: {depths.min():.2f} to {depths.max():.2f} meters")
        
        # Create the projected image
        print("Creating LiDAR projection image...")
        image = create_lidar_image(image_points, depths)
        
        # Add colorbar
        image = add_depth_colorbar(image)
        
        # Add title and info
        cv2.putText(image, 'LiDAR Back-Projection (000002)', (10, 30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(image, f'Points: {len(image_points)} | Max depth: {depths.max():.1f}m', 
                    (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        
        # Save the image
        cv2.imwrite(output_file, image)
        print(f"Saved projection image to: {output_file}")
        
        # Display using matplotlib for better visualization
        plt.figure(figsize=(15, 8))
        plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
        plt.title('LiDAR Points Back-Projected to Camera View')
        plt.axis('off')
        plt.tight_layout()
        plt.savefig('/home/jal.parikh/yolo3d/YOLO3D/lidar_projection_matplotlib.png', 
